...
// Dependency file: contracts/external/uniswap-lib/Babylonian.sol
...
// Dependency file: contracts/external/uniswap-v3-periphery/interfaces/ISwapRouter.sol
...
// Root file: contracts/core/MulRouter.sol
...
// import "contracts/external/uniswap-lib/Babylonian.sol";
// import "contracts/external/uniswap-v3-core/libraries/TickMath.sol";

// import "contracts/external/uniswap-v3-periphery/interfaces/ISwapRouter.sol";
...
contract MulExchange is Ownable, ReentrancyGuard {
	IWETH public WETH;
	IMulFactory public factory;
	...
	uint constant public DISTRIBUTE_PERCENT = 1000;
	uint constant public UNI_FEE = 3000;
	uint constant public UNI_SPACING = 60;
	...
	struct Position {
		...
		uint asset;
		uint interestDebt;
		uint ditributePercent;
		uint liquidity;
		uint tick;
	}

	constructor(IWETH _WETH, IMulFactory _factory, ISwapRouter _uniswapV3Router, INonfungiblePositionManager _uniswapPosManager) {
		WETH = _WETH;
		factory = _factory;
		uniswapV3Router = _uniswapV3Router;
		uniswapPosManager = _uniswapPosManager;
	}
    ...
    function _getTick(address token0, address token1, uint amount0, uint amount1) internal returns (uint tick) {
    	if (token0 > token1) (amount0, amount1) = (tokenB, tokenA);
    	uint sqrtPrice = Babylonian.sqrt(amount0.mul(Q96).div(amount1));
    	tick = TickMath.getTickAtSqrtRatio(sqrtPrice);
    }

	function long(address inputToken, address buyToken, uint input, uint leverage, uint assetMin) external onlyEOA {
		require(leverage >= MAG && leverage <= MAX_LEVERAGE, "INVALID LEVERAGE");

		address pool = factory.getPool(inputToken);
		require(pool != address(0), "NOT SUPPORT NOW");

		uint borrowAmount;
		if(leverage > MAG) {
			borrowAmount = input.mul(leverage.sub(MAG)).div(MAG);
			IMulPool(pool).borrow(borrowAmount);
		}

		uint amount0 = input.add(borrowAmount).mul(10000 + DISTRIBUTE_PERCENT).div(10000);
		uint amount1 = _swapToken(inputToken, buyToken, input.add(borrowAmount), assetMin);

		uint tickLower = _getTick(inputToken, buyToken, amount0, amount1);
		uint tickUpper = tickLower.add(UNI_SPACING);


		uniswapPosManager.mint(inputToken, buyToken, UNI_FEE, tickLower, tickUpper, 0, amount1, 0, amount1, address(this), block.timestamp);
		// Position memory position = Position({true, inputToken, buyToken, input, borrowAmount, })
	}

	function _swapToken(address fromToken, address toToken, uint amountIn, uint amountOutMin) internal returns (uint amountOut){
		ExactInputSingleParams params = ExactInputSingleParams(fromToken, toToken, UNI_FEE, address(this), block.timestamp, amountIn, amountOutMin, 0);
