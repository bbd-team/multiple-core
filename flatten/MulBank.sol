...
// Dependency file: contracts/core/MulERC20.sol
...
// Root file: contracts/core/MulBank.sol
...
// import "contracts/core/MulERC20.sol";
...
contract MulBank is Ownable {
    ...
    event PoolInitialized(address indexed supplyToken, address shareToken, uint pid);
	...
	function withdraw(uint token, uint amount) external returns (uint amount) {
		PoolInfo memory pool = poolInfo[token];
	...
	function borrow(uint token, uint amount, address to) onlyStrategy external {
	...
	function repay(uint token, uint amount, uint interest) onlyStrategy external {
		PoolInfo storage pool = poolInfo[token];
		...
		pool.totalReward = pool.totalReward.add(interest);
	}
	
	function distribute(uint token, uint amount) external {
	    PoolInfo storage pool = poolInfo[token];
		pool.supplyToken.safeTransferFrom(msg.sender, address(this), amount);
		pool.totalReward = pool.totalReward.add(amount);
	}
}